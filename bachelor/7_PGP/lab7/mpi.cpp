            ...
            for(int j = start[1]; j < lims[1]; ++j){
                for(int i = start[0]; i < lims[0]; ++i){
                    inner_buff[i + (j + k*size_y)*size_x] =
                        edge_buffs[orr][i*m[0] + (j*m[1] + k*m[2]*ones[1])*ones[0]];
                }
            }
        }
...
void export_edges(double* inner_buff, double** edge_buffs, int* edge_sizes, int* blocks){
    ...
    for(int orr = 0; orr < 6; orr++) {
        ...
        for(int k = start[2]; k < lims[2]; k++){
            for(int j = start[1]; j < lims[1]; j++){
                for(int i = start[0]; i < lims[0]; i++){
                    edge_buffs[orr][i * m[0] + (j * m[1] + k * m[2] * ones[1]) * ones[0]] = inner_buff[i + (j + k*size_y) * size_x];
                }
            }
        }
        ones[dir] = lims[dir] + 1;
...
enum sides {
    left = 0, right, front, back, down, up
};
...
int main(int argc, char **argv) {
    ...
    if(main_worker) {
        ...
        std::cin >> path >> eps >> l[0] >> l[1] >> l[2] >> u[down] >> u[up];
        std::cin >> u[left] >> u[right] >> u[front] >> u[back]>> u0;
    }
    ...
    do {
        max_diff = 0.0;
        edges_exchange(edge_buff_in, edge_buff_out, dir_edge_sizes, coords, dimens, neighb, grid_comm);
        import_edges(buff0, edge_buff_in, dir_edge_sizes, blocks);

        for(int k = 1; k <= blocks[2]; ++k){
            for(int j = 1; j <= blocks[1]; ++j){
                for(int i = 1; i <= blocks[0]; ++i){
                    ...
                    max_diff = max_determine(buff0[idx(i, j, k)], buff1[idx(i, j, k)], max_diff);
                }
            }
        }
                    ...
                    for(int i = 0; i < dimens[0]; ++i){
                        ...
                        if (main_worker) {
                            if(coords[2] == k && coords[1] == j && coords[0] == i) {
                                ...
                                int rank = 0;
                                MPI_Cart_rank(grid_comm, t_coord, &rank);
